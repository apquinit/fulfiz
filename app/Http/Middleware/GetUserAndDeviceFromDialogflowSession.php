...
class GetUserAndDeviceFromDialogflowSession
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (strpos($session, config('app.dialogflow.irene_lite')) !== false) {
            ...
            Log::info('Get user and device from session', ['Session' => $session, 'Device' => 'Irene Lite Messenger']);
        } elseif (strpos($session, config('app.dialogflow.irene')) !== false) {
            $user = config('app.dialogflow.irene');
            $code = 'irene-messenger';
            Log::info('Get user and device from session', ['Session' => $session, 'Device' => 'Irene Messenger']);
        } else {
            $device = $this->deviceRepository->getByCode($session);
            if ($device->status === 'ENABLED') {
                $user = $device->user_id;
                $code = $device->code;
                Log::info('Get user and device from session', ['Session' => $session, 'Device' => $device->name]);
